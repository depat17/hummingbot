...
def maker_trading_pair_prompt():
    exchange = micro_profit_maker_config_map.get("exchange").value
...
def order_amount_prompt() -> str:
    trading_pair = micro_profit_maker_config_map["market"].value
...
def on_validate_price_source(value: str):
    if value != "external_market":
        micro_profit_maker_config_map["price_source_exchange"].value = None
        micro_profit_maker_config_map["price_source_market"].value = None
        micro_profit_maker_config_map["take_if_crossed"].value = None
    if value != "custom_api":
        micro_profit_maker_config_map["price_source_custom_api"].value = None
    else:
        micro_profit_maker_config_map["price_type"].value = "custom"


def price_source_market_prompt() -> str:
    external_market = micro_profit_maker_config_map.get("price_source_exchange").value
    return f'Enter the token trading pair on {external_market} >>> '


def validate_price_source_exchange(value: str) -> Optional[str]:
    if value == micro_profit_maker_config_map.get("exchange").value:
...
def validate_price_source_market(value: str) -> Optional[str]:
    market = micro_profit_maker_config_map.get("price_source_exchange").value
...
def validate_price_type(value: str) -> Optional[str]:
    error = None
    price_source = micro_profit_maker_config_map.get("price_source").value
...
def on_validated_price_type(value: str):
    if value == 'inventory_cost':
        micro_profit_maker_config_map["inventory_price"].value = None
...
micro_profit_maker_config_map = {
    "strategy":
        ConfigVar(key="strategy",
                  prompt=None,
                  default="micro_profit_maker"),
    "exchange":
    ...
    "price_ceiling_pct":
        ConfigVar(key="price_ceiling_pct",
                  ...
                  type_str="decimal",
                  default=Decimal("1"),
                  required_if=lambda: micro_profit_maker_config_map.get("moving_price_band_enabled").value,
    ...
    "order_level_amount":
        ConfigVar(key="order_level_amount",
                  ...
                  required_if=lambda: micro_profit_maker_config_map.get("order_levels").value > 1,
                  type_str="decimal",
    ...
    "inventory_target_base_pct":
        ConfigVar(key="inventory_target_base_pct",
                  ...
                  required_if=lambda: micro_profit_maker_config_map.get("inventory_skew_enabled").value,
                  type_str="decimal",
    ...
    "inventory_price":
        ConfigVar(key="inventory_price",
                  ...
                  required_if=lambda: micro_profit_maker_config_map.get("price_type").value == "inventory_cost",
                  default=Decimal("1"),
                  ),
    "filled_order_delay":
    ...
    "hanging_orders_cancel_pct":
        ConfigVar(key="hanging_orders_cancel_pct",
                  prompt="At what spread percentage (from mid price) will hanging orders be canceled? "
                         "(Enter 1 to indicate 1%) >>> ",
                  required_if=lambda: micro_profit_maker_config_map.get("hanging_orders_enabled").value,
                  type_str="decimal",
                  default=Decimal("10"),
                  validator=lambda v: validate_decimal(v, 0, 100, inclusive=False)),
    ...
    "ask_order_optimization_depth":
        ConfigVar(key="ask_order_optimization_depth",
                  ...
                  required_if=lambda: micro_profit_maker_config_map.get("order_optimization_enabled").value,
                  type_str="decimal",
    ...
    "price_type":
        ConfigVar(key="price_type",
                  ...
                  type_str="str",
                  required_if=lambda: micro_profit_maker_config_map.get("price_source").value != "custom_api",
                  default="mid_price",
    ...
    "price_source_exchange":
        ConfigVar(key="price_source_exchange",
                  ...
                  required_if=lambda: micro_profit_maker_config_map.get("price_source").value == "external_market",
                  type_str="str",
    ...
    "take_if_crossed":
        ConfigVar(key="take_if_crossed",
                  ...
                  required_if=lambda: micro_profit_maker_config_map.get(
